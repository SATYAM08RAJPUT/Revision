...
const AllDataFile = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [roadmap, setRoadMap] = useState([]);
  const [selectContent, setSelectedContent] = useState(null); // Ensure content state is set to null initially
  const navigate = useNavigate();
  const subContainersRef = useRef([]);
  ...
  const fetchAllTopics = async () => {
    ...
    if (course) {
      const initialTopicKey = Object.keys(course)[0];
      // Set the selected content based on topicId or default to the first topic
      setSelectedContent(course[initialTopicKey][parseInt(topicId) - 1 || 0]);
    }
  };
  ...
  // Filter roadmap based on courseId
  const filterData = roadmap.filter((item) => item.id == courseId);
  ...
  // Handle toggle (expand/collapse) for topic
  const handleToggle = (topicId) => {
    navigate(`/course/${topicId.courseId}/RoadMap/${topicId.id}`);
  };

  // Handle scroll to the sub-topic
  const handleScroll = (index) => {
    subContainersRef.current[index].scrollIntoView({
      behavior: 'smooth',
      block: 'start',
  ...
  useEffect(() => {
    // Scroll to the topic when it's selected or when content changes
    if (selectContent) {
      const topicIndex = topicsData
        .flatMap((item) => item[Object.keys(item)[0]]) // Get all topic data in a flat array
        .findIndex((topic) => topic.id === topicId);

      if (topicIndex >= 0 && subContainersRef.current[topicIndex]) {
        subContainersRef.current[topicIndex].scrollIntoView({
          behavior: 'smooth',
          block: 'start',
        });
      }
    }
  }, [selectContent, topicId, topicsData]);

import { useEffect, useState } from "react"
// import './cssRoadMap.css'
import { useNavigate } from "react-router-dom"
import Header from "../Home/Header/header"

export default function ReactRoadMap() {
    const navigate = useNavigate()
    const [reactRoadMap, setReactRoadMap] = useState([]);
    const [loading , setLoading] = useState(true);

    const fetchReactRoad = async () => {
        const response = await fetch("/api/reactRoadmaps");
        console.log(response)
        const result = await response.json()
        setReactRoadMap(result.reactRoadmaps[0].roadMap)
        setLoading(false);
    }
    useEffect(() => {
        fetchReactRoad()
    }, [])


    const handleTopicClick = (item) => {
        navigate(`/course/${item.courseId}/topics/${item.id}`);
    }

    const handleSubTitle = (event, subitem) => {
        event.stopPropagation()
        console.log(subitem)
        navigate(`/course/${subitem.courseId}/topics/${subitem.id}`);
    }

    
    const handleBack = () =>{
        navigate(`/course`)
    }

    if (loading) {
        return (
            <div style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                height: '100vh',
                width: '100%',
                fontSize: '30px'
            }}>
                <i className="fa fa-spinner fa-spin" style={{ fontSize: "74px" }}></i>
            </div>
        );
    }

    return (
        <>
            <div>
                <Header />

                 <div className="back-button-div">
                    <div>
                        <h1>Road Map:</h1>
                        <br />
                        <h1 style={{color:'hsl(24.6, 95%, 53.1%)', fontSize:'50px'}}>React Road Map</h1>
                    </div>
                    <button className="back-button" onClick={handleBack}>Back</button>
                </div>

                <div className="cssRoaaMap">
                    {reactRoadMap.map((item) => (
                        <div
                            key={item.topicName}
                            className="cssBox-Conatiner"
                            onClick={() => handleTopicClick(item)}
                        >
                            <h2>{item.topicName}</h2>
                            <ul className="order-listRoadMap">
                                {item.subTopics.map((subitem) => (
                                    <li
                                        key={subitem.subtitle}
                                        onClick={(event) => handleSubTitle(event, subitem)}
                                    >
                                        {subitem.subtitle}
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ))}
                </div>
            </div>
        </>
    )
}
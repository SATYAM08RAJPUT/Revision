    ...
    let server = createServer({
        ...
        routes() {
            ...
            this.get('/search', (schema, request) => {
                // const alltopics = schema.topics.all()
                let q = request.queryParams.q.toLowerCase();
                let allData =schema.topics.all().models.map(topic =>topic.attrs);
                let allTopic = allData.forEach(item =>item.topicdata);
                let allTopics = allTopic.map(item =>item.title)
                console.log(allTopic)

                // let filteredItem = schema.topics.topicdata[0].where((item) =>console.log(item))
                // console.log(filteredItem)

                // let filterusers = alltopics.models.map((item) => {
                //     const obj = Object.keys(item.attrs)[0]
                //     const finalObj = item[obj]
                //     const filterTitle = finalObj.filter((item) => {
                //         return item.title.includes(q) || item.content.includes(q)
                //     })
                //     return filterTitle
                // })
                // return filterusers
